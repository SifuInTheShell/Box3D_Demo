...
// Inverse of to_b3_transform: a b3Transform (position + rotation) back to a
// Godot Transform3D. Scale is not represented in b3Transform, so the basis is
// pure rotation. Used to read joint local frames back from the core.
inline Transform3D to_gd_transform(const b3Transform &t) {
	return Transform3D(Basis(to_gd(t.q)), to_gd(t.p));
}
